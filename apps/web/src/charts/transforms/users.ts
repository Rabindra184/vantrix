...
/* "Elapsed (s)", not "Time (s)" — review N01. Every time series' data table
   but telemetry's heads its label column `Elapsed (s)` — telemetry's own
   row labels stay raw milliseconds (`transforms/telemetry.ts`), because its
   rows never divide by 1000 the way this file's and `rates.ts`'s do — and
   the charts share one crosshair group and one `[0, durationMs]` domain: two
   names for one quantity is the drift the finding is about, and here it sat
   on the one chart a reader correlates the others against. It names a
   COLUMN only. The drawn axis is named by `Chart` alone, from the viewer's
   time mode (`Elapsed`, or `Time (GMT+5:30)` over wall-clock ticks). */
