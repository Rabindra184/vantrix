...
**Goal:** A Gradle plugin that opens a Vantrix live run when `gatlingRun` starts, streams `simulation.log` as Gatling writes it, and closes the run when the task ends — so the five-tab live page shows the run while it happens and the finished report when it ends.
...
- **The token is `VANTRIX_TOKEN` and nothing else.** Never a DSL property, never a flag. Sent as `Authorization: Bearer <token>`.
...
- **One Vantrix run per simulation.** A new results directory appearing while one is being tailed means the previous simulation ended: final-flush + close it, open the next.
...
- Locked naming: plugin id `dev.vantrix.gatling`, group `dev.vantrix`, artifact `gatling-gradle-plugin`, version `0.1.0-SNAPSHOT`, package `dev.vantrix.gradle`.
...
**Files:**
- Create: `clients/gatling-gradle/settings.gradle.kts`, `clients/gatling-gradle/build.gradle.kts`, `clients/gatling-gradle/gradle.properties`, `clients/gatling-gradle/src/main/kotlin/dev/vantrix/gradle/VantrixPlugin.kt`, `clients/gatling-gradle/src/test/kotlin/dev/vantrix/gradle/PluginSmokeTest.kt`, wrapper files via `gradle wrapper`
- Modify: `.github/workflows/ci.yml` (new `plugin` job beside the `agent` job)

**Interfaces:**
- Produces: an applyable plugin id `dev.vantrix.gatling`; `./gradlew build` and `./gradlew publishToMavenLocal` green; CI job `plugin` running both; publish to GitHub Packages on pushes to `main` using `GITHUB_TOKEN` (no new secret — same-repo Packages accepts it).
...
`VantrixPlugin.kt` (walking skeleton — Task 5 fills it):
```kotlin
...
```bash
cd clients/gatling-gradle && ./gradlew publishToMavenLocal && ls ~/.m2/repository/dev/vantrix/gatling-gradle-plugin/0.1.0-SNAPSHOT/
```
Expected: a `.jar`, a `.pom`, and the plugin-marker artifact under `~/.m2/repository/dev/vantrix/gatling/`.
...
**Files:**
- Create: `clients/gatling-gradle/src/main/kotlin/dev/vantrix/gradle/VantrixExtension.kt`, `.../ResolvedConfig.kt`
- Test: `clients/gatling-gradle/src/test/kotlin/dev/vantrix/gradle/ResolvedConfigTest.kt`

**Interfaces:**
- Produces: `VantrixExtension` (DSL block `vantrix { }`) with `url`, `environment`, `branch`, `commitSha`, `tickSeconds` (default 5), `uploadIfLiveUnavailable` (default false), `resultsDir`; and `ResolvedConfig.from(ext, env: Map<String,String>): ResolvedConfig?` — pure, testable, returns null (with a reason string in `ResolvedConfig.Problem`) when no URL or no token. Token read ONLY from `env["VANTRIX_TOKEN"]`.
...
- [ ] **Step 5: Commit** — `git commit -m "feat(plugin): vantrix{} config with env fallback; token only ever from VANTRIX_TOKEN"`
...
**Files:**
- Modify: `.../VantrixPlugin.kt`
- Test: `.../VantrixPluginFunctionalTest.kt` (Gradle TestKit + the Task 3 fake server)

**Interfaces:**
- Produces: on apply — registers the `vantrix` extension; `tasks.matching { it.name == "gatlingRun" }.configureEach` adds a `doFirst` that resolves config (a `Missing` reason logs one warning and does nothing else), creates `LiveClient` + `RunTailer`, starts it; registers task `vantrixClose` and sets `gatlingRun.finalizedBy(vantrixClose)` — the finalizer calls `tailer.finish()` and runs **even when gatlingRun fails**, which is the whole reason it is a finalizer and not `doLast`. Every plugin action body is wrapped in a top-level `try/catch(Throwable)` that logs and swallows.

- [ ] **Step 1: Write the failing TestKit tests** — each writes a `settings.gradle.kts` + `build.gradle.kts` into a temp project. The build file registers a FAKE `gatlingRun` (a plain task that writes N bytes into `build/reports/gatling/fake-<millis>/simulation.log`, sleeping between writes), applies our plugin from `pluginClasspath` (via `withPluginClasspath()`), and points `VANTRIX_URL` at the fake server:
...
@Test fun `missing token warns once and the build succeeds untouched`() {
    // no VANTRIX_TOKEN in env passed to runner; assert build SUCCESS,
    // output contains "VANTRIX_TOKEN", fake server saw zero requests
}
...
**Files:**
- Create: `.../BundleUploader.kt`; Modify: `VantrixPlugin.kt` (finalizer branch), `LiveClient.kt` (nothing — uploader is its own small class)
...
**Spec coverage:** §0 boundary → no task executes tests (Task 7 runs Gatling via *Gradle*, the permitted side). §1 partial blocks → Task 4 test 2; 8 KiB expectation documented → Task 8. §2 lifecycle/offsets/4 MiB/no-shutdown-hook → Tasks 3–5 (finalizer, not a JVM hook; kill-9 leaves the sweeper path untouched as designed). §3 one-run-per-sim → Task 4 test 4 + Task 5. §4 config/token/idempotencyKey → Task 2; idempotencyKey generated once per RESULTS DIRECTORY, not once per task execution -- minted fresh in `RunTailer.maybeSwitchDirectory` each time a new simulation directory is opened, because a per-task key would dedupe simulation #2 into simulation #1's already-opened run. The safer, per-directory semantic still protects the case the contract names -- a lost-response retry for the SAME directory resuming its run rather than opening a second -- it just scopes "same open call" to one directory instead of the whole task; `LiveClient.open` itself does not retry in 0.1 (§5), so that reuse-on-retry case is not yet exercised by this codebase. §5 failure table → Tasks 3/5/6 tests map row-for-row; `uploadIfLiveUnavailable` default false → Task 2 test 5 + Task 6 TestKit case. §6 toolchain-first + publish → Task 1. §7 three test layers → Tasks 3–4 (fake server), 5 (TestKit), 7 (real stack). §8 out of scope → no task touches the agent, Maven, or the DataWriter SPI.
